...
const PaymentModeReportScreen = ({ route }) => {
    ...
    const cardTitleData = [
        { title: 'Total Value', method: undefined },
        { title: 'Cash', method: 'CASH' },
        { title: 'Card', method: 'CARD' },
        { title: 'Digital Payment', method: 'DIGITAL PAYMENTS' },
        { title: 'Prepaid Redeemed', method: 'Prepaid' },
        { title: 'Rewards Redeemed', method: 'Rewards' },
    ];

    const cardInitialValue = [
        { value: 0 },
        { mode_of_payment: 'CASH', value: 0 },
        { mode_of_payment: 'CARD', value: 0 },
        { mode_of_payment: 'DIGITAL PAYMENTS', value: 0 },
        { mode_of_payment: 'PREPAID', value: 0 },
        { mode_of_payment: 'REWARDS', value: 0 },
    ];
    ...
    const {
        ...
        onDateChangeDays: (firstDate, secondDate) => {
            dispatch(
                fetchPaymentModeReportForBusiness(
                    0,
                    10,
                    firstDate,
                    secondDate,
        ...
        onFirstCustomRangeSelected: (firstDate, secondDate) => {
            dispatch(
                fetchPaymentModeReportForBusiness(
                    0,
                    10,
                    moment(firstDate, 'ddd, DD MMM YYYY').format('YYYY-MM-DD'),
                    moment(secondDate, 'ddd, DD MMM YYYY').format('YYYY-MM-DD'),
                    query,
                    sortName,
                    getSortOrderKey === 1 ? 'desc' : getSortOrderKey === 2 ? 'asc' : 'desc',
                    modeFilter
    ...
    const parseData = (res) => {
        ...
        const changedData = cardValue.current.map((desired) => {
            const matchingEntry = res.data[0].paymentTotalData.find(
                (item) => item.mode_of_payment === desired.mode_of_payment
            );
    ...
    useEffect(() => {
        const fetchData = () => {
            dispatch(fetchPaymentModeReportForBusiness(0, 10, formatDateYYYYMMDD(0), formatDateYYYYMMDD(0))).then((res) => {
                parseData(res);
            });
        };
        fetchData();
        ...
        <ScrollView style={{ backgroundColor: 'white' }}>
            <View style={{ marginTop: 20, paddingHorizontal: 20 }}>
                <Text style={[TextTheme.bodyMedium, { alignSelf: 'center', marginBottom: 20 }]}>
                    Shows complete list of all sales transactions.
                </Text>
                <DatePicker
                ...
                {cardTitleData.map((item, index) => (
                    <CustomImageTextCard
                        ...
                        containerStyle={[
                            ...
                            currentCardActive === item.title ? styles.selectedCardStyle : null,
                        ]}
            ...
            <SearchBar
                onChangeText={(text) => {
                    setQuery(text); // Update state immediately for UI feedback
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ alignItems: 'center' }}>
                <View style={{ paddingHorizontal: 20 }}>
                    <Table borderStyle={{ borderColor: '#c8e1ff' }}>
                        <Row
                            data={tableHeaderList.map((item, index) => (
                                <SortableHeader
                                    key={item.key}
                                    title={item.title}
                                ...
                                {dataList.map((rowData, rowIndex) => (
                                    <Row
                                        ...
                                        data={rowData.map((cell, cellIndex) => (
                                            <Text style={[styles.text, { paddingHorizontal: 10 }]}>{cell}</Text>
                                        ))}
                                        widthArr={widthArr}
