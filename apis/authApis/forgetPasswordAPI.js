...
export default async function forgetPasswordAPI(email, platform) {
    const BaseURL = process.env.EXPO_PUBLIC_API_URI;
    let isFound = false;
    let response = "something went wrong"

    try {
        response = await axios.post(BaseURL + '/user/forgetPassword', {
            platform: platform,
            userName: email
        })
        isFound = true;
        ...
        isFound = false;
        return "incorrect mobile number / email"
    }
